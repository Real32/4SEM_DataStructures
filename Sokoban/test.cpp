...
//  bool can_move=false;
  ...
  if(map[node->hero.x+x][node->hero.y+y]!='#') //чек шага персонажа
  {
    for(int i=0;i<node->boxes.size();i++) //пробегаем по коробкам
    {
      if(node->hero.x+x==node->boxes[i].x&&node->hero.y+y==node->boxes[i].y)//стоит ли бокс на пути
      {
        box_ind=i;
        if(map[node->boxes[i].x+x][node->boxes[i].y+y]!='#')//Если за боксом стена
        {
          for(int j=0;j<node->boxes.size();j++) //бегам по коробкам
          {
            if(node->boxes[i].x+x==node->boxes[j].x&&node->boxes[i].y+y==node->boxes[j].y) //стоит ли коробка за коробкой
            {
              return;
            }
          }
...
// prov(node);
  for(int i=0; i<nodes.size(); i++)
  {
    if(nodes[i])
    {
      if(nodes[i]->parent && compare(nodes[i]->boxes,nodes[i]->parent->boxes)) //Чек коробок родителя, чтобы не бегать назад
      {
        switch(nodes[i]->step)
        {
          case UP:
...
stack<Node*> set_stack(Node* finished) //просмотр
{
	stack<Node*> path;
	while(finished)
	{
...
time = clock();
  ...
  in.close();
  time = clock() - time;
cout << (double)time/CLOCKS_PER_SEC<<endl; //время выполнения 
  system("pause");
  return 0; 
}