...
unsigned char map[HEIGHT][WIDTH]; //Êàðòà
...
ode::Node(Node* _parent, object _hero, vector<object> _boxes, DIRECTION _step)
{
  parent=_parent;
  up=NULL;
  down=NULL;
...
//  bool can_move=false;
  ...
  if(map[node->hero.x+x][node->hero.y+y]!='#') //÷åê øàãà ïåðñîíàæà
  {
    for(int i=0;i<node->boxes.size();i++) //ïðîáåãàåì ïî êîðîáêàì
    {
      if(node->hero.x+x==node->boxes[i].x&&node->hero.y+y==node->boxes[i].y)//ñòîèò ëè áîêñ íà ïóòè
     {
        box_ind=i;
       if(map[node->boxes[i].x+x][node->boxes[i].y+y]!='#')//Åñëè çà áîêñîì ñòåíà
        {
          for(int j=0;j<node->boxes.size();j++) //áåãàì ïî êîðîáêàì
          {
           if(node->boxes[i].x+x==node->boxes[j].x&&node->boxes[i].y+y==node->boxes[j].y) //ñòîèò ëè êîðîáêà çà êîðîáêîé
            {
              return;
            }
          }
...
// prov(node);
  for(int i=0; i<nodes.size(); i++)
 {
    if(nodes[i])
    {
      if(nodes[i]->parent && compare(nodes[i]->boxes,nodes[i]->parent->boxes)) //×åê êîðîáîê ðîäèòåëÿ, ÷òîáû íå áåãàòü íàçàä
      {
        switch(nodes[i]->step)
       {
          case UP:
...
stack<Node*> set_stack(Node* finished) //ïðîñìîòð
{
	stack<Node*> path;
	while(finished)
	{
...
int main()
{
  ...
  in.close();
  time = clock() - time;
cout << (double)time/CLOCKS_PER_SEC<<endl; //âðåìÿ âûïîëíåíèÿ
  system("pause");
  return 0;
}
